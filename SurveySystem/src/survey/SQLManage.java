...
public class SQLManage {
    ...
    public String getSurveyCode() throws SQLException {
        String surveyCode = null;
        String query = "SELECT surveycode FROM questions LIMIT 1"; // Assuming survey codes are stored in the 'questions' table
        try (Statement statement = con.createStatement();
             ResultSet resultSet = statement.executeQuery(query)) {
            if (resultSet.next()) {
                surveyCode = resultSet.getString("surveycode");
            }
        }
        return surveyCode;
    }
